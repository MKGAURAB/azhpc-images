...
#   - Lustre client (Azure Managed Lustre) - except GB200
#   - DOCA/OFED (InfiniBand drivers)
#   - PMIX (Process Management Interface)
#   - AMD CPU libraries (AOCL, AOCC - NOT GPU) - x86_64 only
#   - Intel MKL - x86_64 only
#   - HPC tuning
#   - Azure NFS helper (except GB200)
#   - HPC diagnostics (except GB200)
#   - Monitoring tools (except GB200)
...
# Install Lustre client for Azure Managed Lustre (skip for GB200 - kernel compatibility)
if [ "$SKU" != "GB200" ]; then
...
# Install AMD CPU optimization libraries (AOCL, AOCC compilers) - x86_64 only
...
# Components that are not supported on GB200 yet
if [[ "$SKU" != "GB200" ]]; then
