...
#   - NVIDIA GB200: Special driver, NVSHMEM, NVLOOM, NVBandwidth
#   - AMD: ROCm, RCCL, Docker
#   - Both: MPI libraries (HPC-X, OpenMPI, MVAPICH, Intel MPI)
#   - Health checks (except GB200)
#   - SKU customizations
#
# Usage:
...
#   SKU: GPU model (e.g., A100, H100, MI300X, GB200)
#
# Example:
#   ./install_layer3_gpu.sh NVIDIA A100
#   ./install_layer3_gpu.sh NVIDIA GB200
...
if [ -z "$1" ] || [ -z "$2" ]; then
    ...
    echo "  SKU: GPU model (e.g., A100, H100, MI300X, GB200)"
    exit 1
fi
...
if [ "$GPU" = "NVIDIA" ]; then
    ...
    if [ "$SKU" = "GB200" ]; then
        # GB200 has special driver and additional components
        echo "Installing GB200-specific NVIDIA driver..."
        ./install_nvidiagpudriver_gb200.sh

        # Install NVSHMEM (Shared Memory library for NVIDIA GPUs)
        echo "Installing NVSHMEM..."
        ./install_nvshmem_gb200.sh

        # Install NVLOOM (NVIDIA GPU communication library)
        ...
        # Standard NVIDIA GPU driver installation
...
# Install Azure/NHC Health Checks (requires GPU platform) - except GB200
if [[ "$SKU" != "GB200" ]]; then
